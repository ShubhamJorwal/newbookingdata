...
//       const token = localStorage.getItem('token');
//       try {
//         const response = await axios.get('https://admin.obwsalon.com/api/customers', {
//           headers: {
//             Authorization: `Bearer ${token}`,
//           },
//         });
...
//   const hadnleCustomers=()=>{
//     setShowCustomerList(e=>!e)
//   }
//   const SelectcustomerClick=(obj)=>{
//     console.log(obj)
...
//     const servicesW = useSelector((state) => state.services.services);
//   const loadingW = useSelector((state) => state.services.loading);
//   const errorW = useSelector((state) => state.services.error);
  
//   useEffect(() => {
//     dispatch(fetchServicesForWomen());
//     // setShowDiv(true);
//   }, [dispatch]);
//   const [ALlservicees, setALlservicees] = useState([]);

//     const showServices = () => {
//     // Render the modal with servicesW
//     console.log('====================================');
//     console.log(servicesW,'i');
//     console.log('====================================');
    
//     setALlservicees(servicesW)
//   };

//   const selectServicesW = (clickedService) => {
//     const selectedData = {
...
//             <Link to={"/services/women2"}>
//               <button >Add Service</button>
//             </Link>
//             <button onClick={handleAddProducts}>Add Product</button>
//             <button onClick={hadnleCustomers}>Add customers</button>
//           </div>
//           <div id="Cus">
...
//       {showCustomerList && <div>
//         {customerList&&customerList.map(e=><h1 key={e.customer_id} onClick={()=>SelectcustomerClick(e)}>{e.first_name}</h1>)}
//         </div>}
//         {loadingW ? (
//         <div>Loading...</div>
//       ) : errorW ? (
//         <div>Error: {errorW}</div>
//       ) : (
//         <div>
//             {ALlservicees.map(e=><h1 onClick={selectServicesW}>{e.service_name}</h1>)}
//         </div>
//       )}
//     </>
//   );
...
// new s1402 function for stylists been dded to the object of rendered list that is  been used to display the function end
  ...
  //s140222222 neww aprochhhh    customerr
  const [showCustomerList, setShowCustomerList] = useState(true);
  const [customerList, setCustomerList] = useState([]);
  ...
  const selected_customer_set_in_local=(obj)=>{
        localStorage.setItem('UserBookingData',JSON.stringify({customer:obj}));
        console.log('====================================');
        console.log(obj);
        console.log('====================================');
        setContactNo(obj.contact_no)
  ...
  return (
    <>
      ...
      <div>
        <div id="lastbtnscheck">
          <Link to={"/checkout/bill/processing"}>
            <button style={{ padding: "1rem 2rem" }} id="secondbtn">
              Book now
            </button>
          </Link>
      ...
      {/* my code s1402 cust  */}
      <p onClick={()=>setShowCustomerList(e=>!e)}>x</p>
      {customerList && <div>{
       customerList.map(e=><h1 onClick={()=>selected_customer_set_in_local(e)} style={{border:'1px solid #ccc'}}>{customerList.id} {e.first_name}</h1>)}
       </div>}
    </>
  );
};
